...
#referenceRct <- read.csv(file.path(system.file("settings", "TargetComparatorList.csv", package = "TroyCohortDiagnostics")), sep = ",")
...
for (i in 1:length(trialNames)) {
  ...
  #colnames(specificationRct) <- c("characteristics", "target", "targetSd", "comparator", "comparatorSd", "tag", "isNa", "targetSize", "comparatorSize", "statistics", "analysisId", "covariateIds")
  ...
  #tt <- troyCreateCovariateSettings(specificationRct)
