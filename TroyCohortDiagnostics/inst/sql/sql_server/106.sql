...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1302398,1322184,40163718,40241186)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1302398,1322184,40163718,40241186)
  and c.invalid_reason is null
...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (36889365,1113348,1113262,1113343,46287399,1113350,1113305,1113307,1113347,1113309)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (36889365,1113348,1113262,1113343,46287399,1113350,1113305,1113307,1113347,1113309)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (40482727,319835,439696,439694,314378,439846,319825)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (40482727,319835,439696,439694,314378,439846,319825)
  and c.invalid_reason is null
...
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 44)
) C
JOIN @cdm_database_schema.PERSON P on C.person_id = P.person_id
WHERE C.drug_era_start_date >= DATEFROMPARTS(2013, 2, 27)
...
(
  SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
  JOIN #Codesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 74)
) C
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-3650,P.START_DATE) AND A.START_DATE <= DATEADD(day,0,P.START_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
...
select C.person_id, C.dose_era_id as event_id, C.dose_era_start_date as start_date, C.dose_era_end_date as end_date,
       CAST(NULL as bigint) as visit_occurrence_id,C.dose_era_start_date as sort_date
from 
(
  select de.* 
  FROM @cdm_database_schema.DOSE_ERA de
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 60)
) C

WHERE c.dose_value > 165.0000
...
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 64)
) C

WHERE C.value_as_number > 2.5000
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 65)
) C
...
(
  SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
  JOIN #Codesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 75)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 67)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 68)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 69)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 71)
) C

WHERE C.value_as_number < 9.0000
-- End Measurement Criteria
...
UNION ALL
select inclusion_rule_id, person_id, event_id from #Inclusion_18) I;
...
{19 != 0}?{
  ...
  WHERE (MG.inclusion_rule_mask = POWER(cast(2 as bigint),19)-1)
}
)
...
FROM (
	...
	JOIN #Codesets cs on cs.codeset_id = 44 AND de.drug_concept_id = cs.concept_id
	...
	FROM @cdm_database_schema.DRUG_EXPOSURE de
	JOIN ctePersons p on de.person_id = p.person_id
	JOIN #Codesets cs on cs.codeset_id = 44 AND de.drug_source_concept_id = cs.concept_id
) E
;
...
from (
  SELECT CAST(0 as int) as rule_sequence UNION ALL SELECT CAST(1 as int) as rule_sequence UNION ALL SELECT CAST(2 as int) as rule_sequence UNION ALL SELECT CAST(3 as int) as rule_sequence UNION ALL SELECT CAST(4 as int) as rule_sequence UNION ALL SELECT CAST(5 as int) as rule_sequence UNION ALL SELECT CAST(6 as int) as rule_sequence UNION ALL SELECT CAST(7 as int) as rule_sequence UNION ALL SELECT CAST(8 as int) as rule_sequence UNION ALL SELECT CAST(9 as int) as rule_sequence UNION ALL SELECT CAST(10 as int) as rule_sequence UNION ALL SELECT CAST(11 as int) as rule_sequence UNION ALL SELECT CAST(12 as int) as rule_sequence UNION ALL SELECT CAST(13 as int) as rule_sequence UNION ALL SELECT CAST(14 as int) as rule_sequence UNION ALL SELECT CAST(15 as int) as rule_sequence UNION ALL SELECT CAST(16 as int) as rule_sequence UNION ALL SELECT CAST(17 as int) as rule_sequence UNION ALL SELECT CAST(18 as int) as rule_sequence
) IR;
