...
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 93)
) C

WHERE C.drug_era_start_date >= DATEFROMPARTS(2013, 11, 26)
...
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 58)
) C
...
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 61)
) C
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 65)
) C
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_ERA de
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 65)
) C

WHERE DATEDIFF(d,C.drug_era_start_date, C.drug_era_end_date) > 730
-- End Drug Era Criteria
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 66)
) C
...
(
  SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
  JOIN #Codesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 73)
) C
...
(
  select po.* 
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
JOIN #Codesets cs on (po.procedure_concept_id = cs.concept_id and cs.codeset_id = 37)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 83)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 84)
) C
...
WHERE (C.value_as_number / NULLIF(C.range_high, 0)) > 2.5000
-- End Measurement Criteria
...
(
  SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
  JOIN #Codesets cs on (co.condition_concept_id = cs.concept_id and cs.codeset_id = 87)
) C
...
FROM (
	...
	JOIN #Codesets cs on cs.codeset_id = 93 AND de.drug_concept_id = cs.concept_id
	...
	FROM @cdm_database_schema.DRUG_EXPOSURE de
	JOIN ctePersons p on de.person_id = p.person_id
	JOIN #Codesets cs on cs.codeset_id = 93 AND de.drug_source_concept_id = cs.concept_id
) E
;
