...
# eGFR
resultTablePooled[nrow(resultTablePooled)+1, c("n")] <- data.frame(covariates$covariates %>% filter(conceptId %like% 3029859, covariateValue < 50) %>% summarise(n = n_distinct(.data$rowId)))
resultTablePooled[nrow(resultTablePooled), c("covariateName")] <- "eGFR < 50"
...
# Medical history: stenosis
stenosis <- c(37111502)
resultTablePooled[nrow(resultTablePooled)+1, c("n")] <- data.frame(covariates$covariates %>% filter(conceptId %in% stenosis) %>% summarise(n = n_distinct(.data$rowId)))
resultTablePooled[nrow(resultTablePooled), c("covariateName")] <- "Medical histroy: stenosis"
...
resultTablePooled[nrow(resultTablePooled)+1, c("n")] <- data.frame(covariates$covariates %>% filter(conceptId %in% pci) %>% summarise(n = n_distinct(.data$rowId)))
...
# Thiazolidinedione
thiaz <- c(21600779)
resultTablePooled[nrow(resultTablePooled)+1, c("n")] <- data.frame(covariates$covariates %>% filter(conceptId %in% thiaz) %>% summarise(n = n_distinct(.data$rowId)))
resultTablePooled[nrow(resultTablePooled), c("covariateName")] <- "Thiazolidinedione"
...
#anticoagulants
aspirin <- c(1112807)
antiplatelets <- c(1322184,40163718,40241186,40252640,1302398, 1310149)
...
resultTablePooled[nrow(resultTablePooled), c("covariateName")] <- "Other platelets"
