...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (40241186,21601004,40163718)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (40241186,21601004,40163718)
  and c.invalid_reason is null
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 56)
) C
