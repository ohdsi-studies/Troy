...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (138387,255848)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (138387,255848)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (197494,4281232)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (197494,4281232)
  and c.invalid_reason is null
...
WHERE YEAR(C.drug_exposure_start_date) - P.year_of_birth >= 21
AND C.ordinal = 1
...
WHERE C.value_as_number > 170.0000
-- End Measurement Criteria
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 50)
) C
...
(
  select po.* 
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
JOIN #Codesets cs on (po.procedure_concept_id = cs.concept_id and cs.codeset_id = 52)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 42)
) C
...
WHERE C.value_as_number > 1.8000
-- End Measurement Criteria
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 46)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 40)
) C
