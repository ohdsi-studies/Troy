...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1580747)
...
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 59)
) C

WHERE C.drug_era_start_date >= DATEFROMPARTS(2007, 9, 21)
AND C.drug_exposure_count >= 7
...
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 57)
) C
...
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 60)
) C


-- End Drug Exposure Criteria

) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-91,P.START_DATE) AND A.START_DATE <= DATEADD(day,-1,P.START_DATE) ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-365,P.START_DATE) AND A.START_DATE <= P.OP_END_DATE ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-91,P.START_DATE) AND A.START_DATE <= DATEADD(day,-1,P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
...
FROM (
	...
	JOIN #Codesets cs on cs.codeset_id = 59 AND de.drug_concept_id = cs.concept_id
	...
	FROM @cdm_database_schema.DRUG_EXPOSURE de
	JOIN ctePersons p on de.person_id = p.person_id
	JOIN #Codesets cs on cs.codeset_id = 59 AND de.drug_source_concept_id = cs.concept_id
) E
;
