...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (37398809)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (37398809)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (3022192)
...
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 116)
) C

WHERE C.drug_era_start_date >= DATEFROMPARTS(2011, 1, 24)
...
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 80)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 85)
) C

WHERE C.value_as_number > 130.0000
AND C.unit_concept_id in (9570)
-- End Measurement Criteria
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 86)
) C
...
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 89)
) C
...
JOIN #Codesets cs on (o.observation_concept_id = cs.concept_id and cs.codeset_id = 90)
) C
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 114)
) C
...
JOIN #Codesets cs on (po.procedure_concept_id = cs.concept_id and cs.codeset_id = 101)
) C
...
(
  select po.* 
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
JOIN #Codesets cs on (po.procedure_concept_id = cs.concept_id and cs.codeset_id = 102)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 103)
) C
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 106)
) C
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_ERA de
where de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 109)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 110)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 111)
) C
...
(
  select m.* 
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 113)
) C
...
FROM (
	...
	JOIN #Codesets cs on cs.codeset_id = 116 AND de.drug_concept_id = cs.concept_id
	...
	FROM @cdm_database_schema.DRUG_EXPOSURE de
	JOIN ctePersons p on de.person_id = p.person_id
	JOIN #Codesets cs on cs.codeset_id = 116 AND de.drug_source_concept_id = cs.concept_id
) E
;
...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cs on (de.drug_concept_id = cs.concept_id and cs.codeset_id = 54)
) C
